...
if ( 'post' === $wpcp_post_type || ( 'post' === get_post_type( get_the_ID() ) ) ) {
	...
	if ( ! empty( $categories ) ) {
		foreach ( $categories as $key => $category ) {
			$wpcp_post_cat_term .= '<li class="post-categories"><a href="' . esc_url( get_category_link( $category->term_id ) ) . '" alt="' . /* translators: %s: Category name of the post */
			esc_attr( sprintf( __( 'View all posts in %s', 'wp-carousel-pro' ), $category->name ) ) . '">' . esc_html( $category->name );
...
if ( $show_post_date || $show_post_author || $show_post_tags || $show_post_comment || $show_post_category ) {
	...
	<ul class="wpcp-post-meta">
		<?php echo wp_kses_post( $post_cat_term . ' ' . $underline . $post_author_name . $post_date . $post_tags . $wpcp_comments ); ?>
	</ul>
	<?php
}
