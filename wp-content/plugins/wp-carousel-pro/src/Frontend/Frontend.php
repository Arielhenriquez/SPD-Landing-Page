...
class Frontend {
	 ...
	 * @return void
	 */
	public static function wpcp_db_options_update( $post_id, $get_page_data ) {
	...
	/**
	 * Delete page shortcode ids entry from the centralized option on post save.
	 *
	...
	public function delete_page_wpcp_option_on_save( $post_ID ) {
		...
		// Get all stored data (an array of page_id => shortcode IDs).
		$all_data = get_option( $option_key, array() );

		// Remove the entry for this page/post ID if exists.
		if ( isset( $all_data[ $post_ID ] ) ) {
