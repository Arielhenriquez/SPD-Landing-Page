	...
	class Helper {
		...
		public static function image_attr( $img_id, $image_sizes, $is_variable_width, $carousel_mode, $image_width, $image_height, $image_crop = false, $show_2x_image = false, $watermark = false ) {
			...
			if ( $watermark && function_exists( 'imagecreatefromstring' ) ) {
				$watermarked_image = self::wpcp_watermark( $image_src );
				if ( $watermarked_image && isset( $watermarked_image['url'] ) ) {
					$image_src = $watermarked_image['url'];
				}

				if ( $image_resize_2x_url ) {
					$watermarked_2x = self::wpcp_watermark( $image_resize_2x_url );
					if ( $watermarked_2x && isset( $watermarked_2x['url'] ) ) {
						$image_resize_2x_url = $watermarked_2x['url'];
					}
				}
			}
			return array(
		 ...
		 * @param  string $video_slug video_slug.
		 * @return string
		 */
		public static function image_tag( $lazy_load_image, $carousel_mode, $image_attr, $image_alt_title, $image_title_attr, $lazy_load_img, $wpcp_layout, $video_slug = '' ) {
			$src_2x     = ( ! empty( $image_attr['srcset'] ) ) ? esc_attr( $image_attr['src'] ) . ', ' . esc_attr( $image_attr['srcset'] ) . ' 2x' : esc_attr( $image_attr['src'] );
			$video_slug = ! empty( $video_slug ) ? 'data-item-slug="' . $video_slug . '"' : '';
			if ( 'false' !== $lazy_load_image && 'ticker' !== $carousel_mode && 'carousel' === $wpcp_layout ) {
				$image = sprintf( '<img data-srcset="%6$s" class="wcp-lazy" data-src="%1$s" src="%1$s" %2$s alt="%3$s" width="%4$s" height="%5$s" %7$s loading="lazy">', $image_attr['src'], $image_title_attr, $image_alt_title, $image_attr['width'], $image_attr['height'], $src_2x, $video_slug );
			} elseif ( ! self::is_carousel( $wpcp_layout ) && 'false' !== $lazy_load_image ) {
				$image = sprintf( '<img srcset="%7$s" class="wpcp-lazyload" data-wpcp_src="%1$s" src="%2$s" %3$s alt="%4$s" width="%5$s" height="%6$s" %8$s>', $image_attr['src'], $lazy_load_img, $image_title_attr, $image_alt_title, $image_attr['width'], $image_attr['height'], $src_2x, $video_slug );
			} else {
				$image = sprintf( '<img srcset="%6$s" src="%1$s" %2$s alt="%3$s" width="%4$s" height="%5$s" %7$s class="skip-lazy">', $image_attr['src'], $image_title_attr, $image_alt_title, $image_attr['width'], $image_attr['height'], $src_2x, $video_slug );
			}
			return $image;
		}
		...
		public static function get_pagination( $upload_data, $shortcode_data, $post_id, $pages, $posts_found ) {
			...
			if ( $wpcp_pagination && 'carousel' !== $wpcp_layout ) {
				...
				if ( 'external-carousel' === $carousel_type ) {
					...
					$all_rss_data_count = is_array( $rss_items ) ? count( $rss_items ) : 0;
		...
		public static function wpcp_query( $upload_data, $shortcode_data, $post_id ) {
			...
			if ( 'product-carousel' === $carousel_type ) {
				...
				$show_out_of_stock_product   = apply_filters( 'sp_wpcp_out_of_stock_product', false );
				$meta_query                  = array();
				...
				if ( 'latest' === $wpcp_product_from ) {
					$args = $default_args;
				} elseif ( 'featured_products' === $wpcp_product_from ) {
					$args['tax_query'][] = array(
		...
		public static function get_item_loops( $upload_data, $shortcode_data, $post_id, $animation_class, $wpcp_query = null, $slider_animation = 'slider', $thumbnail_slider = false ) {
			...
			if ( 'video-carousel' === $carousel_type ) {
				...
				$video_autoplay  = isset( $shortcode_data['video_autoplay_mode'] ) && $shortcode_data['video_autoplay_mode'] ? 1 : 0;
				$video_loop      = isset( $shortcode_data['video_loop'] ) && $shortcode_data['video_loop'] ? 1 : 0;
			...
			if ( 'external-carousel' === $carousel_type ) {
				...
				$video_play_mode           = isset( $shortcode_data['video_play_mode'] ) ? $shortcode_data['video_play_mode'] : 'lightbox';
				$wpcp_external_source_type = isset( $upload_data['wpcp_external_source_type'] ) ? $upload_data['wpcp_external_source_type'] : 'rss_feed';
				...
				$rss_items         = self::rss_feeds( $upload_data, $show_total_content );
				$youtube_rss_items = self::rss_feeds( $upload_data, $show_total_content );
				$rss_items         = 'rss_feed' === $wpcp_external_source_type ? $rss_items : $youtube_rss_items;
		...
		public static function rss_feeds( $upload_data, $show_total_content ) {
			...
			$wpcp_external_source_type = isset( $upload_data['wpcp_external_source_type'] ) ? $upload_data['wpcp_external_source_type'] : 'rss_feed';
			$wpcp_youtube_channel_id   = isset( $upload_data['wpcp_youtube_channel_id'] ) ? $upload_data['wpcp_youtube_channel_id'] : '';
			$youtube_feed_url          = isset( $wpcp_youtube_channel_id ) && ! empty( $wpcp_youtube_channel_id ) ? 'https://www.youtube.com/feeds/videos.xml?channel_id=' . $wpcp_youtube_channel_id : '';
			$feeds_url                 = 'rss_feed' === $wpcp_external_source_type && isset( $upload_data['wpcp_feeds_url'] ) ? $upload_data['wpcp_feeds_url'] : $youtube_feed_url;
		...
		public static function sp_wpcp_html_show( $upload_data, $shortcode_data, $post_id, $main_section_title, $is_preview = false ) {
			...
			if ( self::is_carousel( $wpcp_layout ) ) {
				...
				// Carousel Row.
				$carousel_row = ! empty( $shortcode_data['wpcp_carousel_row']['lg_desktop'] ) ? $shortcode_data['wpcp_carousel_row'] : array(
					'lg_desktop' => '2',
					'desktop'    => '2',
					'laptop'     => '2',
					'tablet'     => '2',
			...
			if ( ( 'product-carousel' === $carousel_type && $light_box ) || 'external-carousel' === $carousel_type || 'video-carousel' === $carousel_type || ( 'image-carousel' === $carousel_type && 'l_box' === $image_link_show ) ) {
			...
			$lightbox_data = 'data-infobar="' . esc_attr( $show_lightbox_image_counter ) . '" data-thumbs="' . esc_attr( $l_box_thumb_visibility ) . '" data-protect_image="' . esc_attr( $l_box_protect_image ) . '" data-autoplay="' . esc_attr( $l_box_autoplay ) . '" data-loop="' . esc_attr( $l_box_loop ) . '" data-speed="' . esc_attr( $l_box_autoplay_speed ) . '" data-sliding_effect="' . esc_attr( $l_box_sliding_effect ) . '" data-open_close="' . esc_attr( $l_box_open_close_effect ) . '" data-outside="' . esc_attr( $l_box_outside_close ) . '" data-keyboard="' . esc_attr( $l_box_keyboard_nav ) . '"  data-l_box_img_sharpe="' . esc_attr( $show_l_box_img_sharper_on_retina ) . '"';
			...
			foreach ( $_video_sources as $_video_source ) {
				...
				switch ( $video_type ) {
					...
					case 'dailymotion':
						...
						break;
					case 'sproutvideo':
						// Get the SproutVideo video thumbnail and embed URL.
						$sproutvideo_video_thumbnail = isset( $_video_source['sproutvideo_video_thumbnail']['url'] ) ? $_video_source['sproutvideo_video_thumbnail']['url'] : '';
						$wpcp_video_url              = isset( $_video_source['sproutvideo_video_embed_url'] ) ? $_video_source['sproutvideo_video_embed_url'] : '';

						// Video thumbnail.
						if ( ! empty( $sproutvideo_video_thumbnail ) ) {
							$wpcp_video_thumb_url = $sproutvideo_video_thumbnail;
						} else {
							$wpcp_video_thumb_url = 'https://placehold.co/600x400.png';
						}
						break;
					case 'self_hosted':
			...
			switch ( $video_type ) {
				...
				case 'wistia':
					...
					if ( empty( $wpcp_video_thumb_url ) && ! empty( $wpcp_video_id ) ) {
						...
						$wistia_video_width   = apply_filters( 'wp_carousel_pro_wistia_thumb_width', '960' );
						$wistia_embed_data    = wp_remote_get( "https://fast.wistia.net/oembed?url=https://home.wistia.com/medias/$wpcp_video_id?embedType=async&videoWidth=$wistia_video_width", $wistia_args );
						$wistia_data          = json_decode( wp_remote_retrieve_body( $wistia_embed_data ), true );
						$wpcp_video_thumb_url = isset( $wistia_data['thumbnail_url'] ) ? $wistia_data['thumbnail_url'] : '';
					}
					break;
		...
		public static function get_vimeo_video_data( $video_id ) {
			...
			// Determine if it's a unlisted video (contains '/').
			$is_unlisted = strpos( $video_id, '/' ) !== false;
