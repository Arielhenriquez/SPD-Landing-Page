...
if ( ! class_exists( 'Gutenberg_Block_Init' ) ) {
	...
	class Gutenberg_Block_Init {
		...
		public function sp_wp_carousel_pro_render_shortcode( $attributes ) {
			...
			return '<div id="' . uniqid() . '" ' . $class_name . ' ><a href="' . esc_url( $edit_page_link ) . '" target="_blank" class="sp_wp_carousel_block_edit_button">Edit View</a>' . do_shortcode( '[sp_wpcarousel id="' . sanitize_text_field( $attributes['shortcode'] ) . '" is_admin="' . esc_attr( $attributes['is_admin'] ) . '"]' ) . '</div>';
		}
	}
}
