...
if ( ! class_exists( 'SP_WPCP_Framework_Field_column' ) ) {
	...
	class SP_WPCP_Framework_Field_column extends SP_WPCP_Framework_Fields {
		...
		public function render() {
			...
			$min  = ( isset( $args['min'] ) ) ? ' required min="' . $args['min'] . '"' : '';
			$step = ( isset( $args['step'] ) ) ? ' step="any"' : '';
			if ( ! empty( $args['all'] ) ) {
				...
				foreach ( $properties as $property ) {
					...
					echo '<input type="number" name="' . esc_attr( $this->field_name( '[' . $property . ']' ) ) . '" value="' . esc_attr( $value[ $property ] ) . '"' . wp_kses_post( $placeholder . $min . $step ) . ' class="sp_wpcp-number" />';
					echo ( count( $args['units'] ) === 1 && ! empty( $args['unit'] ) ) ? '<span class="sp_wpcp--label sp_wpcp--unit">' . esc_attr( $args['units'][0] ) . '</span>' : '';
					echo '</div>';
