...
; (function ($, window, document, undefined) {
	...
	// Custom Dependency for backend tabs.
	let lastState = {};
	function getActiveValues() {
		return {
			layout: $(".wpcp_carousel_type .sp_wpcp--image.sp_wpcp--active input").val(),
			preset: $(".wpcp_layout .sp_wpcp--image.sp_wpcp--active input").val(),
			carouselStyle: $(".wpcp_carousel_mode .sp_wpcp--image.sp_wpcp--active input").val(),
			sliderStyle: $(".wpcp_slider_style .sp_wpcp--image.sp_wpcp--active input").val(),
			clickAction: $(".wpcp_logo_link_show_class .sp_wpcp--image.sp_wpcp--active input").val()
		};
	}

	function updateLayoutSettings(state) {
		// Prevent infinite recursion by checking if values actually changed
		if (JSON.stringify(state) === JSON.stringify(lastState)) return;
		lastState = { ...state }; // Update stored state

		const { layout, preset, carouselStyle, sliderStyle, clickAction } = state;
		// Determine which settings tabs should be shown based on the selected layout.
		const postStyleTabs = ['external-carousel', 'post-carousel'].includes(layout);
		const imgStyleTabs = ['audio-carousel', 'content-carousel'].includes(layout);
		const hasJustifiedStyle = ['image-carousel', 'video-carousel'].includes(layout);
		const hasLightbox = ['image-carousel', 'mix-content'].includes(layout);
		const titleDescTabs = ['image-carousel', 'video-carousel', 'audio-carousel'].includes(layout);
		// Toggle visibility of settings tabs based on layout selection.
		$('.sp_wpcp-tabbed-nav .post-content').toggle(postStyleTabs);
		$('.sp_wpcp-tabbed-nav .image-styles').toggle(!imgStyleTabs);
		$('.sp_wpcp-tabbed-nav .title-description').toggle(titleDescTabs);
		$('.sp_wpcp-tabbed-nav .product-info').toggle(layout === 'product-carousel');
		$('.wpcp_layout .sp_wpcp--image.justified').toggle(hasJustifiedStyle);
		$('.sp_wpcp-nav .lightbox-settings').toggle(hasLightbox && 'l_box' === clickAction);

		// --- Preset Dependencies ---
		const hasCarouselPagination = ['carousel', 'slider'].includes(preset);
		const movingItemStyle = ['carousel', 'slider', 'justified'].includes(preset);
		const isCarousel = ['carousel', 'slider', 'thumbnails-slider'].includes(preset);
		const isCarouselStyle = ['ticker', 'triple'].includes(carouselStyle);
		const showNavPagination = (preset === 'carousel' && isCarouselStyle) || (preset === 'slider' && sliderStyle === 'fashion');

		const hideImgZoom = preset === 'slider' && ['shaders', 'slicer', 'fashion', 'shutters'].includes(sliderStyle);

		// Toggle settings based on the current preset and dependencies.
		$('.sp_wpcp-nav .carousel-settings').toggle(isCarousel);

		// Pagination
		$('.carousel-pagination').toggle(
			hasCarouselPagination && !showNavPagination && preset !== 'thumbnails-slider'
		);
		...
		$('.wpcp_content_style .sp_wpcp--image.moving').toggle(!movingItemStyle);
		$('.wpcp_image_zoom').toggle(!hideImgZoom);

		// --- Additional Dependencies ---

		// Hide 'l_box' option when post type source are active.
		$('.wpcp_logo_link_show_class .l_box').toggle(!postStyleTabs);

		// If 'postStyleTabs' is active and 'l_box' is clicked, switch to 'link'.
		if (postStyleTabs && 'l_box' === clickAction) {
			$('.wpcp_logo_link_show_class .link').trigger('click');
		}
		// If 'mix-content' layout is selected and 'l_box' is not clicked, trigger 'l_box' click.
		if ('mix-content' === layout && 'l_box' !== clickAction) {
			$('.wpcp_logo_link_show_class .l_box').trigger('click');
		}
		// If 'thumbnails-slider' preset is selected, automatically switch to 'standard' mode.
		if ('thumbnails-slider' === preset) {
			$('.wpcp_carousel_mode .standard').trigger('click');
		}
		// --- Ensure the correct settings tab is active ---
		// If the lightbox tab is active but should be hidden, switch to 'General Settings'.
		if (!hasLightbox && $('.sp_wpcp-nav .lightbox-settings a').hasClass('sp_wpcp-active')) {
			$('.sp_wpcp-nav .general-settings a').trigger('click');
		}
		// If an invalid tab is currently active, switch to 'Item Styles'.
		if ((imgStyleTabs && $('.sp_wpcp-tabbed-nav .image-styles').hasClass('sp_wpcp-tabbed-active')) || (!postStyleTabs && $('.sp_wpcp-tabbed-nav .post-content').hasClass('sp_wpcp-tabbed-active')) || (layout !== 'product-carousel' && $('.sp_wpcp-tabbed-nav .product-info').hasClass('sp_wpcp-tabbed-active')) || (!titleDescTabs && $('.sp_wpcp-tabbed-nav .title-description').hasClass('sp_wpcp-tabbed-active'))) {
			$('.sp_wpcp-tabbed-nav .item-styles').trigger('click');
		}

		// If navigation or pagination is active but should be hidden, switch to 'Carousel Basics'.
		if ((hasCarouselPagination && showNavPagination) && ($('.carousel-navigation').hasClass('sp_wpcp-tabbed-active') || $('.carousel-pagination').hasClass('sp_wpcp-tabbed-active'))) {
			$('.carousel-basics').trigger('click');
		}
	}
	$('.sp_wpcp-nav-inline ul li').on('click', function (e) {
		e.preventDefault();
		$(document).find('.sp_wpcp-tabbed-nav a:first').trigger('click');
	});

	// Initialize with active values.
	updateLayoutSettings(getActiveValues());

	// Common event handler for all dependency updates.
	$(document).on("click", ".wpcp_carousel_type .sp_wpcp--image, .wpcp_layout .sp_wpcp--image, .wpcp_carousel_mode .sp_wpcp--image, .wpcp_slider_style .sp_wpcp--image,.wpcp_logo_link_show_class .sp_wpcp--image", function (event) {
		event.stopPropagation();
		updateLayoutSettings(getActiveValues());
	});
...
; (function ($) {
	...
	// YouTube Channel ID Generator.
	$(document).find('.wpcp-youtube-channel-id-generator').on('click', async function () {
		const handle = prompt("Enter YouTube handle (e.g. @ShapedPlugin):");
		if (!handle) return;

		// Get API key from settings input
		const apiKey = $("input[name='sp_wpcp_upload_options[wpcp_youtube_api_key]']").val();
		if (!apiKey) {
			alert("YouTube API key not found in settings");
			return;
		}
		...
		try {
			...
			if (data.items && data.items.length > 0) {
				...
				// Set channel ID in the field
				$("input[name='sp_wpcp_upload_options[wpcp_youtube_channel_id]']").val(channelId);

				alert("Channel ID: " + channelId + "\nClick OK to set it in the field.");
			} else {
				alert("Channel ID not found");
			}
		} catch (err) {
			alert("Error fetching channel ID");
			console.error(err);
		}
	});

	/**
	 * Toggles video-related fields based on selected source and carousel type.
	 */
	function toggleVideoFields() {
		// Get selected values
		let selectedSource = $('input[name="sp_wpcp_upload_options[wpcp_external_source_type]"]:checked').val();
		let selectedCarousel = $('input[name="sp_wpcp_upload_options[wpcp_carousel_type]"]:checked').val();

		// Select the video-related fields
		let videoFields = $(
			'.sp_wpcp-field-subheading[data-controller="wpcp_carousel_type"],' +
			'.sp_wpcp-field-button_set.video_play_mode,' +
			'.sp_wpcp-field-spinner[data-controller="wpcp_carousel_type"],' +
			'.sp_wpcp-field-color[data-controller="wpcp_carousel_type"]'
		);

		if ( selectedSource === 'youtube_feed' || selectedCarousel === 'video-carousel' || selectedCarousel === 'mix-content' ) {
			videoFields.show();
		} else {
			videoFields.hide();
		}
	...
	// Toggle on change.
	$(document).on('change', 'input[name="sp_wpcp_upload_options[wpcp_external_source_type]"], input[name="sp_wpcp_upload_options[wpcp_carousel_type]"]', function () {
		toggleVideoFields();
	});
