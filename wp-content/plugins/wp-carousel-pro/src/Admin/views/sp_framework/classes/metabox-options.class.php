...
if ( ! class_exists( 'SP_WPCP_Framework_Metabox' ) ) {
	...
	class SP_WPCP_Framework_Metabox extends SP_WPCP_Framework_Abstract {
		...
		public function add_meta_box_content( $post, $callback ) {
			...
			if ( $has_nav ) {
				...
				foreach ( $this->sections as $section ) {
					...
					$tab_class = isset( $section['class'] ) ? $section['class'] : '';

					echo '<li class="' . esc_attr( $tab_class ) . '"><a href="#" data-section="' . esc_attr( $this->unique ) . '_' . esc_attr( $tab_key ) . '">' . wp_kses_post( $tab_icon . $section['title'] . $tab_error ) . '</a></li>';
		...
		/**
		 * Modifies .htaccess to increase max_input_vars.
		 */
		public static function wpcp_modify_htaccess() {
			// Ensure we are on an Apache server.
			...
			if ( ! file_exists( $htaccess_path ) || ! is_writable( $htaccess_path ) ) {
				error_log( 'WP Carousel: .htaccess is missing or not writable.' );
				return;
			}
			...
			$rule_pattern = '/# WP Carousel Max Input Vars\s*\nphp_value max_input_vars \d+/';

			// New rule to set.
			$new_rule = "# WP Carousel Max Input Vars\nphp_value max_input_vars " . WPCAROUSEL_MAX_INPUT_VARS;

			// If the rule already exists, replace it; otherwise, append it.
			if ( preg_match( $rule_pattern, $htaccess_content ) ) {
				$htaccess_content = preg_replace( $rule_pattern, $new_rule, $htaccess_content );
			} else {
				$htaccess_content .= "\n" . $new_rule . "\n";
			}
			...
			if ( ! $wp_filesystem || ! is_object( $wp_filesystem ) ) {
				error_log( 'WP Carousel: WP_Filesystem could not be initialized.' );
				return;
			}

			// Write back the modified .htaccess content.
			if ( ! $wp_filesystem->put_contents( $htaccess_path, $htaccess_content, FS_CHMOD_FILE ) ) {
				error_log( 'WP Carousel: Failed to write to .htaccess.' );
			}
		}
	}
}
