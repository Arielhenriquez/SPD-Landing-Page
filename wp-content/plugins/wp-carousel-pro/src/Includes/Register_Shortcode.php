...
class Register_Shortcode {
	...
	public function sp_wp_carousel_shortcode( $attributes ) {
		...
		$force_enqueue     = wpcp_get_option( 'wpcp_carousel_in_ajax_theme', false );
		...
		if ( $should_enqueue ) {
			...
			$is_admin       = ! empty( $attributes['is_admin'] ) ? true : false;
			// For the Block theme frontend page.
			if ( $is_block_theme && ! $is_admin ) {
				wp_add_inline_style( 'wp-carousel-pro', $dynamic_css );
			} else {
				echo '<style id="sp_wpcp_dynamic_css' . esc_attr( $post_id ) . '">' . wp_strip_all_tags( $dynamic_css ) . '</style>'; // phpcs:ignore
			}
		...
		if ( ! ( $force_enqueue || $is_admin ) ) {
			Frontend::wpcp_db_options_update( $post_id, $get_page_data );
		}
