	...
	class Image_Watermark {
		...
		public function __construct( $wm_img, $props = array(), $wm_img_title = '', $watermark_type = 'logo' ) {
			...
			if ( 'logo' === $this->wm_type ) {
				...
				if ( ! in_array( $this->wm_mime, array( 'image/jpeg', 'image/png', 'image/gif', 'image/webp' ) ) ) {
		...
		public function mark_it( $img_src ) {
			...
			if ( @file_exists( $extless_path . '.jpg' ) ) {
				...
				$exists         = true;
			} elseif ( @file_exists( $extless_path . '.webp' ) ) {
				$this->img_mime = 'image/webp';
				$exists         = true;
			}
			...
			$headers = get_headers( $img_src, 1 );
			if ( ! $headers || strpos( $headers[0], '200' ) === false ) {
				$this->throw_notice( 'Remote image (' . $img_src . ') not found', __LINE__ );
				return false;
			}

			// Try to get MIME type from headers.
			if ( isset( $headers['Content-Type'] ) ) {
				$this->img_mime = is_array( $headers['Content-Type'] ) ? $headers['Content-Type'][0] : $headers['Content-Type'];
			} else {
				// Try to determine MIME type from file extension .
				$file_info = pathinfo( $img_src );
				...
				$this->img_mime = $mime_map[ $extension ] ?? null;
			}

			// Check mime.
			if ( ! in_array( $this->img_mime, array( 'image/jpeg', 'image/png', 'image/gif', 'image/webp' ), true ) ) {
				$this->throw_notice( 'Remote image (' . $img_src . ') has unsupported type: ' . $this->img_mime, __LINE__ );
				return false;
			}
		...
		private function wpcp_add_watermark_text( $img_src ) {
			$image = new \Imagick();
			if ( ! in_array( 'PNG', $image->queryFormats(), true ) ) {
				return array( 'url' => $img_src );
			}
			...
			if ( $this->is_url( $img_src ) ) {
				...
				switch ( $this->img_mime ) {
					...
					case 'image/webp':  // Added WebP support
						$image = imagecreatefromwebp( $img_src );
						break;
					default:
						$image = false;
				}
			...
			if ( false !== $image ) {
				...
				switch ( $this->img_mime ) {
					...
					case 'image/gif':
						...
						break;
					case 'image/webp':  // Added WebP saving.
		...
		private function wpcp_add_watermark_image( $image ) {
			switch ( $this->wm_mime ) {
				...
				case 'image/png':
					...
					break;
				case 'image/webp':
					$watermark = imagecreatefromwebp( $this->wm_img_path );
					break;
